...
/**
 * 
 * 데이터베이스 GAME_VIEWER 테이블을 내용을 추가 또는 변경해주는 클래스
 * 
 * @author Kwon
 *
 */
public class GameViewerModifier {
	
	private GameViewerModifier(){}
	
	private static final void createTableIfNotExists( Connection conn, Statement stmt ) throws Exception{
		
		stmt.execute(
				"CREATE TABLE IF NOT EXISTS billiard_counter.GAME_VIEWER("
	...
	public static final boolean saveNewGameViewer( int tableNumber, int row, int col ){
		...
		try{
			...
			sql = "INSERT INTO billiard_counter.GAME_VIEWER VALUES(" + tableNumber + ", " + row + ", " + col + ", FALSE);";
	...
	public static final boolean deleteGameViewer( int tableNumber ) {
		...
		try {
			...
			Connection conn = BilliardCounterConnector.getConnection();
			Statement stmt = conn.createStatement();
			String sql = "DELETE FROM billiard_counter.GAME_VIEWER "
	...
	/**
	 * 
	 * tableNumber의 당구대 뷰어를 사용중인 상태로 변경해주는 매서드
	 * 
	 * @param tableNumber 변경하고자하는 당구대 번호
	 * @return 작업이 정상적으로 처리되면 true, 그렇지 않으면 false
	 */
	public static final boolean useThisGameViewer( int tableNumber ) {
		
		try {
			
			Connection conn = BilliardCounterConnector.getConnection();
			Statement stmt = conn.createStatement();
			String sql = "UPDATE billiard_counter.GAME_VIEWER "
					+ "SET IS_IN_USE = TRUE "
					+ "WHERE TABLE_NUMBER = " + tableNumber + ";";
	...
	/**
	 * 
	 * tableNumber의 당구대 뷰어를 사용중이지 않은 상태로 변경해주는 매서드
	 * 
	 * @param tableNumber 변경하고자하는 당구대 번호
	 * @return 작업이 정상적으로 처리되면 true, 그렇지 않으면 false
	 */
	public static final boolean endUseThisGameViewer( int tableNumber ) {
